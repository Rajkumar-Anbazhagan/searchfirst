...
if (isset($_POST['add'])) {
    ...
    $mysqli->query("INSERT INTO course_category (branch, created_at) VALUES ('$branch', NOW())");
    header('Location: course_category.php'); exit;
}
// Handle Edit
if (isset($_POST['edit'])) {
    $id = (int)$_POST['id'];
    $branch = $mysqli->real_escape_string($_POST['branch']);
    $mysqli->query("UPDATE course_category SET branch = '$branch' WHERE id = $id");
    header('Location: course_category.php'); exit;
}
...
if (isset($_POST['delete'])) {
    ...
    if (is_array($ids)) {
        ...
        $ids_str = implode(',', $ids);
        $mysqli->query("DELETE FROM course_category WHERE id IN ($ids_str)");
    } elseif (is_numeric($ids)) {
        $id = (int)$ids;
        $mysqli->query("DELETE FROM course_category WHERE id=$id");
    }
    header('Location: course_category.php'); exit;
}
$types = $mysqli->query("SELECT * FROM course_category ORDER BY id ASC");
?>
    ...
    <div class="dashboard-main-wrapper">
        ...
        <div class="dashboard-body">
                    <div class="card mt-24">
                        <div class="card-body">
        <h4>Course Category Management</h4>
        <form method="post" class="mb-3 d-flex gap-2">
            <input type="text" name="branch" class="form-control w-auto" placeholder="New Course Category" required>
            <button type="submit" name="add" class="btn btn-success">Add</button>
        </form>
        <form method="post" id="deleteForm">
            <div class="table-responsive mt-12" style="max-height: 500px; overflow-y: auto;">
                <table id="branchTable" class="table table-bordered table-striped align-middle text-center mt-12 mb-12" style="font-size:15px;">
                    ...
                    <?php while($row = $types->fetch_assoc()): ?>
                        <tr>
                            ...
                            <td>
                                <button type="button" class="btn btn-sm btn-primary editBtn me-1" data-id="<?= $row['id'] ?>" data-branch="<?= htmlspecialchars($row['branch']) ?>">Edit</button>
                                <button type="submit" name="delete" value="1" class="btn btn-sm btn-danger singleDeleteBtn" onclick="return confirm('Delete this course category?')" formaction="?ids[]=<?= $row['id'] ?>">Delete</button>
                            </td>
                        </tr>
                    <?php endwhile; ?>
                    </tbody>
