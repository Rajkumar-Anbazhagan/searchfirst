...
if (isset($_POST['add'])) {
    ...
    $mysqli->query("INSERT INTO academic_year (year_name, created_at) VALUES ('$year_name', NOW())");
    header('Location: academic_year.php'); exit;
}
// Handle Edit
if (isset($_POST['edit'])) {
    $id = (int)$_POST['id'];
    $year_name = $mysqli->real_escape_string($_POST['year_name']);
    $mysqli->query("UPDATE academic_year SET year_name='$year_name' WHERE id=$id");
    header('Location: academic_year.php'); exit;
}
...
if (isset($_POST['delete'])) {
    ...
    if (is_array($ids)) {
        ...
        $ids_str = implode(',', $ids);
        $mysqli->query("DELETE FROM academic_year WHERE id IN ($ids_str)");
    }
    header('Location: academic_year.php'); exit;
}
$years = $mysqli->query("SELECT * FROM academic_year ORDER BY id ASC");
?>
...
<li class="sidebar-menu__item has-dropdown">
    ...
    <div class="dashboard-main-wrapper">
        ...
        <form method="post" class="mb-3 d-flex gap-2">
            <input type="text" name="year_name" class="form-control w-auto" placeholder="New Academic Year" required>
            <button type="submit" name="add" class="btn btn-success">Add</button>
        </form>
        <form method="post" id="deleteForm">
            <div class="table-responsive mt-12" style="max-height: 500px; overflow-y: auto;">
                <table id="yearTable" class="table table-bordered table-striped align-middle text-center mt-12 mb-12" style="font-size:15px;">
                    ...
                    <?php while($row = $years->fetch_assoc()): ?>
                        <tr>
                            ...
                            <td>
                                <button type="button" class="btn btn-sm btn-primary editBtn me-1" data-id="<?= $row['id'] ?>" data-year_name="<?= htmlspecialchars($row['year_name']) ?>">Edit</button>
                                <button type="submit" name="delete" value="1" class="btn btn-sm btn-danger singleDeleteBtn" onclick="return confirm('Delete this academic year?')" formaction="?ids[]=<?=$row['id']?>">Delete</button>
                            </td>
                        </tr>
                    <?php endwhile; ?>
                    </tbody>
            ...
            <button type="submit" name="delete" value="1" class="btn btn-danger mt-2" onclick="return confirm('Delete selected academic years?')">Delete Selected</button>
        </form>
    <!-- Edit Modal -->
        ...
        <div class="modal-content">
          <form method="post">
            ...
            <div class="modal-body">
              ...
              <input type="text" name="year_name" id="editYearName" class="form-control" required>
            </div>
...
.btn-sm {
    ...
    $(function(){
        ...
        $('.editBtn').on('click', function(){
            ...
            $('#editYearName').val($(this).data('year_name'));
