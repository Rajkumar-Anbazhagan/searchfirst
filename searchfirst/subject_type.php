...
if (isset($_POST['add'])) {
    ...
    $mysqli->query("INSERT INTO subject_type (subject_name, created_at) VALUES ('$subject_name', NOW())");
    header('Location: subject_type.php'); exit;
}
// Handle Edit
if (isset($_POST['edit'])) {
    $id = (int)$_POST['id'];
    $subject_name = $mysqli->real_escape_string($_POST['subject_name']);
    $mysqli->query("UPDATE subject_type SET subject_name='$subject_name' WHERE id=$id");
    header('Location: subject_type.php'); exit;
}
...
if (isset($_POST['delete'])) {
    ...
    if (is_array($ids)) {
        ...
        $ids_str = implode(',', $ids);
        $mysqli->query("DELETE FROM subject_type WHERE id IN ($ids_str)");
    }
    header('Location: subject_type.php'); exit;
}
$types = $mysqli->query("SELECT * FROM subject_type ORDER BY id ASC");
?>
    ...
    <div class="dashboard-main-wrapper">
        ...
        <div class="dashboard-body">
                    <div class="card mt-24">
                        <div class="card-body">
<h4>Subject Type Management</h4>
        <form method="post" class="mb-3 d-flex gap-2">
            <input type="text" name="subject_name" class="form-control w-auto" placeholder="New Subject Type" required>
            <button type="submit" name="add" class="btn btn-success">Add</button>
        </form>
        <form method="post" id="deleteForm">
            <div class="table-responsive mt-12">
            <table id="typeTable" class="table table-bordered table-striped mt-12 mb-12">
                <thead>
                    <tr>
                ...
                <?php while($row = $types->fetch_assoc()): ?>
                    <tr>
                        ...
                        <td>
                            <button type="button" class="btn btn-sm btn-primary editBtn" data-id="<?= $row['id'] ?>" data-subject_name="<?= htmlspecialchars($row['subject_name']) ?>">Edit</button>
                            <button type="submit" name="delete" value="1" class="btn btn-sm btn-danger singleDeleteBtn" onclick="return confirm('Delete this subject type?')" formaction="?ids[]=<?=$row['id']?>">Delete</button>
                        </td>
                    </tr>
                <?php endwhile; ?>
                </tbody>
            ...
            <button type="submit" name="delete" value="1" class="btn btn-danger mt-2" onclick="return confirm('Delete selected subject types?')">Delete Selected</button>
        </form>
     <!-- Edit Modal -->
...
.btn-sm {
    ...
    $(function(){
        ...
        $('.editBtn').on('click', function(){
            ...
            $('#editSubjectName').val($(this).data('subject_name'));
