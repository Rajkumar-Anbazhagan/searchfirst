...
if (isset($_POST['add'])) {
    ...
    $fields = [
        'institution_type', 'code', 'name', 'contact_number', 'principal', 'password',
        'address', 'city', 'district', 'pincode', 'website', 'email',
        'startup_year', 'establishment_year', 'affiliation'
    ];
    $data = [];
    foreach ($fields as $f) {
        $data[$f] = $mysqli->real_escape_string($_POST[$f] ?? '');
    }
    // Handle file uploads
    $institution_logo = '';
    $institution_photo = '';
    if (!empty($_FILES['institution_logo']['name'])) {
        $institution_logo = uniqid('logo_') . '_' . basename($_FILES['institution_logo']['name']);
        $logo_target = 'institution_profile/' . $institution_logo;
        if (!move_uploaded_file($_FILES['institution_logo']['tmp_name'], $logo_target)) {
            echo '<div style="color:red">Failed to upload institution_logo to ' . $logo_target . '</div>';
            var_dump($_FILES['institution_logo']);
            exit;
        }
    }
    if (!empty($_FILES['institution_photo']['name'])) {
        $institution_photo = uniqid('photo_') . '_' . basename($_FILES['institution_photo']['name']);
        $photo_target = 'institution_profile/' . $institution_photo;
        if (!move_uploaded_file($_FILES['institution_photo']['tmp_name'], $photo_target)) {
            echo '<div style="color:red">Failed to upload institution_photo to ' . $photo_target . '</div>';
            var_dump($_FILES['institution_photo']);
            exit;
        }
    }
    $sql = "INSERT INTO add_institution (
        institution_type, code, name, contact_number, principal, password,
        address, city, district, pincode, website, email,
        institution_logo, startup_year, establishment_year, institution_photo, affiliation, created_at
    ) VALUES (
        '{$data['institution_type']}', '{$data['code']}', '{$data['name']}', '{$data['contact_number']}', '{$data['principal']}', '{$data['password']}',
        '{$data['address']}', '{$data['city']}', '{$data['district']}', '{$data['pincode']}', '{$data['website']}', '{$data['email']}',
        '$institution_logo', '{$data['startup_year']}', '{$data['establishment_year']}', '$institution_photo', '{$data['affiliation']}', NOW()
    )";
    if (!$mysqli->query($sql)) {
        echo '<div style="color:red">MySQL Error: ' . $mysqli->error . '</div>';
        echo '<pre>' . htmlspecialchars($sql) . '</pre>';
        exit;
    }
    header('Location: add_institution.php'); exit;
}
// Handle Edit
if (isset($_POST['edit'])) {
    $id = (int)$_POST['id'];
    ...
    $set = [];
    foreach ($fields as $f) {
        $val = $mysqli->real_escape_string($_POST[$f] ?? '');
        $set[] = "$f='$val'";
    }
    // Handle file uploads
    if (!empty($_FILES['institution_logo']['name'])) {
        $institution_logo = uniqid('logo_') . '_' . basename($_FILES['institution_logo']['name']);
        move_uploaded_file($_FILES['institution_logo']['tmp_name'], 'institution_profile/' . $institution_logo);
        $set[] = "institution_logo='$institution_logo'";
    }
    if (!empty($_FILES['institution_photo']['name'])) {
        $institution_photo = uniqid('photo_') . '_' . basename($_FILES['institution_photo']['name']);
        move_uploaded_file($_FILES['institution_photo']['tmp_name'], 'institution_profile/' . $institution_photo);
        $set[] = "institution_photo='$institution_photo'";
    }
    $sql = "UPDATE add_institution SET ".implode(',', $set)." WHERE id=$id";
    $mysqli->query($sql);
    header('Location: add_institution.php'); exit;
}
...
if (isset($_POST['delete'])) {
    ...
    if (is_array($ids)) {
        ...
        $ids_str = implode(',', $ids);
        $mysqli->query("DELETE FROM add_institution WHERE id IN ($ids_str)");
    } elseif (is_numeric($ids)) {
        $id = (int)$ids;
        $mysqli->query("DELETE FROM add_institution WHERE id=$id");
    }
    header('Location: add_institution.php'); exit;
}
// Fetch institution types and districts for dropdowns
$institutions = $mysqli->query("SELECT * FROM add_institution ORDER BY id ASC");
$institution_types = $mysqli->query("SELECT * FROM institution_type ORDER BY type ASC");
$districts = $mysqli->query("SELECT * FROM district ORDER BY name ASC");

// Prepare arrays for dropdowns
$institution_type_options = [];
while($row = $institution_types->fetch_assoc()) {
    $institution_type_options[] = $row['type'];
}
$district_options = [];
while($row = $districts->fetch_assoc()) {
    $district_options[] = $row['name'];
}
?>
...
<li class="sidebar-menu__item has-dropdown">
    ...
    <div class="dashboard-main-wrapper">
        ...
        <button class="btn btn-success mb-3" data-bs-toggle="modal" data-bs-target="#addModal">Add Institution</button>
                <form method="post" id="deleteForm">
            <div class="table-responsive mt-12" style="max-height: 500px; overflow-y: auto;">
                <table id="institutionTable" class="table table-bordered table-striped align-middle text-center mt-12 mb-12" style="font-size:15px;">
    ...
    <tr>
        ...
        <td><?= $row['id'] ?></td>
        <td><?= htmlspecialchars($row['institution_type']) ?></td>
        <td><?= htmlspecialchars($row['code']) ?></td>
        <td><?= htmlspecialchars($row['name']) ?></td>
        <td><?= htmlspecialchars($row['contact_number']) ?></td>
        <td><?= htmlspecialchars($row['principal']) ?></td>
        <td><?php if($row['institution_logo']): ?><img src="institution_profile/<?= htmlspecialchars($row['institution_logo']) ?>" alt="logo" width="40"><?php endif; ?></td>
        <td><?php if($row['institution_photo']): ?><img src="institution_profile/<?= htmlspecialchars($row['institution_photo']) ?>" alt="photo" width="40"><?php endif; ?></td>
        <td><?= $row['created_at'] ?></td>
        <td>
            <button type="button" class="btn btn-sm btn-primary editBtn me-1" data-row='<?= json_encode($row) ?>'>Edit</button>
            <button type="submit" name="delete" value="1" class="btn btn-sm btn-danger singleDeleteBtn" onclick="return confirm('Delete this institution?')" formaction="?ids[]=<?= $row['id'] ?>">Delete</button>
        </td>
    </tr>
<?php endwhile; ?>
                    </tbody>
            ...
            <button type="submit" name="delete" value="1" class="btn btn-danger mt-2" onclick="return confirm('Delete selected institutions?')">Delete Selected</button>
        </form>
        <!-- Add Modal -->
              ...
              <form method="post" enctype="multipart/form-data">
                ...
                <div class="modal-body">
                  <ul class="nav nav-tabs" id="addTab" role="tablist">
                    <li class="nav-item"><a class="nav-link active" data-bs-toggle="tab" href="#addBasic">Basic</a></li>
                    <li class="nav-item"><a class="nav-link" data-bs-toggle="tab" href="#addContact">Contact</a></li>
                    <li class="nav-item"><a class="nav-link" data-bs-toggle="tab" href="#addProfile">Profile</a></li>
                  </ul>
                  <div class="tab-content mt-3">
                    <div class="tab-pane fade show active" id="addBasic">
                      <div class="row mb-2">
                        <div class="col-md-4">Institution Type</div>
                        <div class="col-md-8">
                          <select name="institution_type" class="form-control" required>
                            <option value="">Select Institution Type</option>
                            <?php foreach($institution_type_options as $type): ?>
                      ...
                      <div class="row mb-2">
                        ...
                        <div class="col-md-8"><input type="text" name="code" class="form-control" required></div>
                      </div>
                      <div class="row mb-2">
                      ...
                      <div class="row mb-2">
                        ...
                        <div class="col-md-8"><input type="number" name="contact_number" class="form-control"></div>
                      </div>
                      <div class="row mb-2">
                        <div class="col-md-4">Principal</div>
                        <div class="col-md-8"><input type="text" name="principal" class="form-control"></div>
                      </div>
                      <div class="row mb-2">
                    ...
                    <div class="tab-pane fade" id="addContact">
                      <div class="row mb-2">
                        <div class="col-md-4">Address</div>
                        <div class="col-md-8"><input type="text" name="address" class="form-control"></div>
                      </div>
                      <div class="row mb-2">
                        <div class="col-md-4">City</div>
                        <div class="col-md-8"><input type="text" name="city" class="form-control"></div>
                      </div>
                      <div class="row mb-2">
                      ...
                      <div class="row mb-2">
                        ...
                        <div class="col-md-8"><input type="text" name="pincode" class="form-control"></div>
                      </div>
                      <div class="row mb-2">
                        <div class="col-md-4">Website</div>
                        <div class="col-md-8"><input type="text" name="website" class="form-control"></div>
                      </div>
                      <div class="row mb-2">
                    ...
                    <div class="tab-pane fade" id="addProfile">
                      <div class="row mb-2">
                        <div class="col-md-4">Institution Logo</div>
                        <div class="col-md-8"><input type="file" name="institution_logo" class="form-control"></div>
                      </div>
                      <div class="row mb-2">
                        <div class="col-md-4">Startup Year</div>
                        <div class="col-md-8"><input type="text" name="startup_year" class="form-control"></div>
                      </div>
                      <div class="row mb-2">
                        <div class="col-md-4">Establishment Year</div>
                        <div class="col-md-8"><input type="text" name="establishment_year" class="form-control"></div>
                      </div>
                      <div class="row mb-2">
                        <div class="col-md-4">Institution Photo</div>
                        <div class="col-md-8"><input type="file" name="institution_photo" class="form-control"></div>
                      </div>
                      <div class="row mb-2">
                        <div class="col-md-4">Affiliation</div>
                        <div class="col-md-8"><input type="text" name="affiliation" class="form-control"></div>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <form method="post" enctype="multipart/form-data">
                ...
                <div class="modal-body">
                  <ul class="nav nav-tabs" id="editTab" role="tablist">
                    <li class="nav-item"><a class="nav-link active" data-bs-toggle="tab" href="#editBasic">Basic</a></li>
                    <li class="nav-item"><a class="nav-link" data-bs-toggle="tab" href="#editContact">Contact</a></li>
                    <li class="nav-item"><a class="nav-link" data-bs-toggle="tab" href="#editProfile">Profile</a></li>
                  </ul>
                  <div class="tab-content mt-3">
                    <div class="tab-pane fade show active" id="editBasic">
                      <div class="row mb-2">
                        <div class="col-md-4">Institution Type</div>
                        <div class="col-md-8">
                          <select name="institution_type" id="editInstitutionType" class="form-control" required>
                            <option value="">Select Institution Type</option>
                            <?php foreach($institution_type_options as $type): ?>
                              <option value="<?= htmlspecialchars($type) ?>"><?= htmlspecialchars($type) ?></option>
                            <?php endforeach; ?>
                      ...
                      <div class="row mb-2">
                        <div class="col-md-4">Code *</div>
                        <div class="col-md-8"><input type="text" name="code" id="editCode" class="form-control" required></div>
                      </div>
                      <div class="row mb-2">
                        <div class="col-md-4">Name</div>
                        <div class="col-md-8"><input type="text" name="name" id="editName" class="form-control"></div>
                      </div>
                      <div class="row mb-2">
                        <div class="col-md-4">Contact Number</div>
                        <div class="col-md-8"><input type="number" name="contact_number" id="editContactNumber" class="form-control"></div>
                      </div>
                      <div class="row mb-2">
                        <div class="col-md-4">Principal</div>
                        <div class="col-md-8"><input type="text" name="principal" id="editPrincipal" class="form-control"></div>
                      </div>
                      <div class="row mb-2">
                        <div class="col-md-4">Password</div>
                        <div class="col-md-8"><input type="password" name="password" id="editPassword" class="form-control"></div>
                      </div>
                    </div>
                    <div class="tab-pane fade" id="editContact">
                      <div class="row mb-2">
                        <div class="col-md-4">Address</div>
                        <div class="col-md-8"><input type="text" name="address" id="editAddress" class="form-control"></div>
                      </div>
                      <div class="row mb-2">
                        <div class="col-md-4">City</div>
                        <div class="col-md-8"><input type="text" name="city" id="editCity" class="form-control"></div>
                      </div>
                      <div class="row mb-2">
                        <div class="col-md-4">District</div>
                        <div class="col-md-8">
                      ...
                      <div class="row mb-2">
                        <div class="col-md-4">Pincode</div>
                        <div class="col-md-8"><input type="text" name="pincode" id="editPincode" class="form-control"></div>
                      </div>
                      <div class="row mb-2">
                        <div class="col-md-4">Website</div>
                        <div class="col-md-8"><input type="text" name="website" id="editWebsite" class="form-control"></div>
                      </div>
                      <div class="row mb-2">
                        <div class="col-md-4">E Mail</div>
                        <div class="col-md-8"><input type="email" name="email" id="editEmail" class="form-control"></div>
                      </div>
                    </div>
                    <div class="tab-pane fade" id="editProfile">
                      ...
                      <div class="row mb-2">
                        <div class="col-md-4">Startup Year</div>
                        <div class="col-md-8"><input type="text" name="startup_year" id="editStartupYear" class="form-control"></div>
                      </div>
                      <div class="row mb-2">
                        <div class="col-md-4">Establishment Year</div>
                        <div class="col-md-8"><input type="text" name="establishment_year" id="editEstablishmentYear" class="form-control"></div>
                      </div>
                      <div class="row mb-2">
                        <div class="col-md-4">Institution Photo</div>
                        <div class="col-md-8"><input type="file" name="institution_photo" class="form-control"></div>
                      ...
                      <div class="row mb-2">
                        <div class="col-md-4">Affiliation</div>
                        <div class="col-md-8"><input type="text" name="affiliation" id="editAffiliation" class="form-control"></div>
                      </div>
                    </div>
                  </div>
                </div>
    ...
    $(function(){
        ...
        $('.editBtn').on('click', function(){
            ...
            $('#editId').val(row.id);
            $('#editInstitutionType').val(row.institution_type).change();
            $('#editCode').val(row.code);
            $('#editName').val(row.name);
            $('#editContactNumber').val(row.contact_number);
            $('#editPrincipal').val(row.principal);
            $('#editPassword').val(row.password);
            $('#editAddress').val(row.address);
            $('#editCity').val(row.city);
            $('#editDistrict').val(row.district).change();
            $('#editPincode').val(row.pincode);
            $('#editWebsite').val(row.website);
            $('#editEmail').val(row.email);
            $('#editStartupYear').val(row.startup_year);
            $('#editEstablishmentYear').val(row.establishment_year);
            $('#editAffiliation').val(row.affiliation);
...
<?php
if (!is_dir('institution_profile')) mkdir('institution_profile', 0777, true);
?>