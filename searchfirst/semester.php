...
if (isset($_POST['add'])) {
    ...
    $mysqli->query("INSERT INTO semester (semester, created_at) VALUES ('$semester', NOW())");
    header('Location: semester.php'); exit;
}
// Handle Edit
if (isset($_POST['edit'])) {
    $id = (int)$_POST['id'];
    $semester = $mysqli->real_escape_string($_POST['semester']);
    $mysqli->query("UPDATE semester SET semester='$semester' WHERE id=$id");
    header('Location: semester.php'); exit;
}
...
if (isset($_POST['delete'])) {
    ...
    if (is_array($ids)) {
        ...
        $ids_str = implode(',', $ids);
        $mysqli->query("DELETE FROM semester WHERE id IN ($ids_str)");
    }
    header('Location: semester.php'); exit;
}
$semesters = $mysqli->query("SELECT * FROM semester ORDER BY id ASC");
?>
...
<li class="sidebar-menu__item has-dropdown">
    ...
    <div class="dashboard-main-wrapper">
        ...
        <form method="post" class="mb-3 d-flex gap-2">
            <input type="text" name="semester" class="form-control w-auto" placeholder="New Semester" required>
            <button type="submit" name="add" class="btn btn-success">Add</button>
        </form>
        <form method="post" id="deleteForm">
            <div class="table-responsive mt-12" style="max-height: 500px; overflow-y: auto;">
            <table id="semesterTable" class="table table-bordered table-striped mt-12 mb-12">
                <thead>
                    <tr>
                ...
                <?php while($row = $semesters->fetch_assoc()): ?>
                    <tr>
                        ...
                        <td>
                            <button type="button" class="btn btn-sm btn-primary editBtn" data-id="<?= $row['id'] ?>" data-semester="<?= htmlspecialchars($row['semester']) ?>">Edit</button>
                            <button type="submit" name="delete" value="1" class="btn btn-sm btn-danger singleDeleteBtn" onclick="return confirm('Delete this semester?')" formaction="?ids[]=<?=$row['id']?>">Delete</button>
                        </td>
                    </tr>
                <?php endwhile; ?>
                </tbody>
            ...
            <button type="submit" name="delete" value="1" class="btn btn-danger mt-2" onclick="return confirm('Delete selected semesters?')">Delete Selected</button>
        </form>
    <!-- Edit Modal -->
...
.btn-sm {
    ...
    $(function(){
        ...
        $('.editBtn').on('click', function(){
            ...
            $('#editSemester').val($(this).data('semester'));
