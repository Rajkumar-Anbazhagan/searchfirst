...
if (isset($_POST['add'])) {
    ...
    $contact_details = $mysqli->real_escape_string($_POST['contact_details']);
    $email = $mysqli->real_escape_string($_POST['email']);
    $mysqli->query("INSERT INTO exam_officers (username, password, designation, profile, contact_details, email, created_at) VALUES ('$username', '$password', '$designation', '$profile', '$contact_details', '$email', NOW())");
    header('Location: exam_officers.php'); exit;
}
// Handle Edit
if (isset($_POST['edit'])) {
    $id = (int)$_POST['id'];
    ...
    $contact_details = $mysqli->real_escape_string($_POST['contact_details']);
    $email = $mysqli->real_escape_string($_POST['email']);
    $mysqli->query("UPDATE exam_officers SET username='$username', password='$password', designation='$designation', profile='$profile', contact_details='$contact_details', email='$email' WHERE id=$id");
    header('Location: exam_officers.php'); exit;
}
...
if (isset($_POST['delete'])) {
    ...
    if (is_array($ids)) {
        ...
        $ids_str = implode(',', $ids);
        $mysqli->query("DELETE FROM exam_officers WHERE id IN ($ids_str)");
    }
    header('Location: exam_officers.php'); exit;
}
$officers = $mysqli->query("SELECT * FROM exam_officers ORDER BY id ASC");
?>
...
<head>
    ...
    <!-- Title -->
    <title> Exam Officers Management</title>
    <!-- Favicon -->
    ...
    <div class="dashboard-main-wrapper">
        ...
        <div class="dashboard-body">
                    <div class="card mt-24">
                        <div class="card-body">
                <h4>Exam Officers Management</h4>
        <form method="post" class="mb-3 row g-2 align-items-end">
            ...
            <div class="col"><input type="text" name="password" class="form-control" placeholder="Password" required></div>
            <div class="col"><input type="text" name="designation" class="form-control" placeholder="Designation" required></div>
            <div class="col"><input type="text" name="profile" class="form-control" placeholder="Profile Image" required></div>
            <div class="col"><input type="text" name="contact_details" class="form-control" placeholder="Contact Details" required></div>
            <div class="col"><input type="email" name="email" class="form-control" placeholder="Email" required></div>
            <div class="col-auto"><button type="submit" name="add" class="btn btn-success">Add</button></div>
        </form>
        <form method="post" id="deleteForm">
            <div class="table-responsive mt-12" style="max-height: 500px; overflow-y: auto;">
            <table id="officerTable" class="table table-bordered table-striped mt-12 mb-12">
                <thead>
                    <tr>
                ...
                <?php while($row = $officers->fetch_assoc()): ?>
                    <tr>
                        <td style="text-align: left;"><input type="checkbox" name="ids[]" value="<?= $row['id'] ?>"></td>
                        <td><?= $row['id'] ?></td>
                        ...
                        <td>
                            <button type="button" class="btn btn-sm btn-primary editBtn" 
                                ...
                                data-profile="<?= htmlspecialchars($row['profile']) ?>"
                                data-contact_details="<?= htmlspecialchars($row['contact_details']) ?>"
                                data-email="<?= htmlspecialchars($row['email']) ?>"
                            >Edit</button>
                            <button type="submit" name="delete" value="1" class="btn btn-sm btn-danger singleDeleteBtn" onclick="return confirm('Delete this exam officer?')" formaction="?ids[]=<?=$row['id']?>">Delete</button>
                        </td>
                    </tr>
                <?php endwhile; ?>
                </tbody>
            ...
            <button type="submit" name="delete" value="1" class="btn btn-danger mt-2" onclick="return confirm('Delete selected exam officers?')">Delete Selected</button>
        </form>
    <!-- Edit Modal -->
        ...
        <div class="modal-content">
          <form method="post">
            <div class="modal-header">
              <h5 class="modal-title">Edit Exam Officer</h5>
              <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body row g-2">
              <input type="hidden" name="id" id="editId">
              <div class="col-12 mb-2"><input type="text" name="username" id="editUsername" class="form-control" placeholder="Username" required></div>
              <div class="col-12 mb-2"><input type="text" name="password" id="editPassword" class="form-control" placeholder="Password" required></div>
              <div class="col-12 mb-2"><input type="text" name="designation" id="editDesignation" class="form-control" placeholder="Designation" required></div>
              <div class="col-12 mb-2"><input type="text" name="profile" id="editProfile" class="form-control" placeholder="Profile Image" required></div>
              <div class="col-12 mb-2"><input type="text" name="contact_details" id="editContactDetails" class="form-control" placeholder="Contact Details" required></div>
              <div class="col-12 mb-2"><input type="email" name="email" id="editEmail" class="form-control" placeholder="Email" required></div>
            </div>
...
.btn-sm {
    ...
    $(function(){
        ...
        $('.editBtn').on('click', function(){
            ...
            $('#editContactDetails').val($(this).data('contact_details'));
