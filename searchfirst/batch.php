...
if (isset($_POST['add'])) {
    ...
    $mysqli->query("INSERT INTO batch (batch_name, created_at) VALUES ('$batch_name', NOW())");
    header('Location: batch.php'); exit;
}
// Handle Edit
if (isset($_POST['edit'])) {
    $id = (int)$_POST['id'];
    $batch_name = $mysqli->real_escape_string($_POST['batch_name']);
    $mysqli->query("UPDATE batch SET batch_name='$batch_name' WHERE id=$id");
    header('Location: batch.php'); exit;
}
...
if (isset($_POST['delete'])) {
    ...
    if (is_array($ids)) {
        ...
        $ids_str = implode(',', $ids);
        $mysqli->query("DELETE FROM batch WHERE id IN ($ids_str)");
    }
    header('Location: batch.php'); exit;
}
$batches = $mysqli->query("SELECT * FROM batch ORDER BY id ASC");
?>
    ...
    <div class="dashboard-main-wrapper">
       ...
       <h4>Batch Management</h4>
        <form method="post" class="mb-3 d-flex gap-2">
            <input type="text" name="batch_name" class="form-control w-auto" placeholder="New Batch" required>
            <button type="submit" name="add" class="btn btn-success">Add</button>
        </form>
                <form method="post" id="deleteForm">
                    <div class="table-responsive mt-12" style="max-height: 500px; overflow-y: auto;">
            <table id="batchTable" class="table table-bordered table-striped mt-12 mb-12">
                <thead>
                    <tr>
                ...
                <?php while($row = $batches->fetch_assoc()): ?>
                    <tr>
                        ...
                        <td>
                            <button type="button" class="btn btn-sm btn-primary editBtn" data-id="<?= $row['id'] ?>" data-batch_name="<?= htmlspecialchars($row['batch_name']) ?>">Edit</button>
                            <button type="submit" name="delete" value="1" class="btn btn-sm btn-danger singleDeleteBtn" onclick="return confirm('Delete this batch?')" formaction="?ids[]=<?=$row['id']?>">Delete</button>
                        </td>
                    </tr>
                <?php endwhile; ?>
                </tbody>
            ...
            <button type="submit" name="delete" value="1" class="btn btn-danger mt-2" onclick="return confirm('Delete selected batches?')">Delete Selected</button>
        </form>
    <!-- Edit Modal -->
...
.btn-sm {
    ...
    $(function(){
        ...
        $('.editBtn').on('click', function(){
            ...
            $('#editBatchName').val($(this).data('batch_name'));
