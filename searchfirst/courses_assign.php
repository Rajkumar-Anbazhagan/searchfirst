...
if (isset($_POST['add'])) {
    ...
    $academic_year = $mysqli->real_escape_string($_POST['academic_year']);
    $batch = $mysqli->real_escape_string($_POST['batch']);
    $institution_code = $mysqli->real_escape_string($_POST['institution_code']);
    $course_code = $mysqli->real_escape_string($_POST['course_code']);
    $mysqli->query("INSERT INTO course_allocation (regulation, academic_year, batch, institution, course, created_at) VALUES ('$regulation', '$academic_year', '$batch', '$institution_code', '$course_code', NOW())");
    header('Location: courses_assign.php'); exit;
}
// Handle Edit
if (isset($_POST['edit'])) {
    $id = (int)$_POST['id'];
    ...
    $mysqli->query("UPDATE course_allocation SET regulation='$regulation', academic_year='$academic_year', batch='$batch', institution='$institution_code', course='$course_code', created_at=NOW() WHERE id=$id");
    header('Location: courses_assign.php'); exit;
}
...
if (isset($_POST['delete'])) {
    ...
    if (is_array($ids)) {
        ...
        $ids_str = implode(',', $ids);
        $mysqli->query("DELETE FROM course_allocation WHERE id IN ($ids_str)");
    } elseif (is_numeric($ids)) {
        $id = (int)$ids;
        $mysqli->query("DELETE FROM course_allocation WHERE id=$id");
    }
    header('Location: courses_assign.php'); exit;
}
$allocs = $mysqli->query("SELECT * FROM course_allocation ORDER BY id ASC");
$regulations = $mysqli->query("SELECT * FROM regulation ORDER BY regulation ASC");
$years = $mysqli->query("SELECT * FROM academic_year ORDER BY year_name ASC");
$batches = $mysqli->query("SELECT * FROM batch ORDER BY batch_name ASC");
$institutions = $mysqli->query("SELECT * FROM add_institution ORDER BY code ASC");
$courses = $mysqli->query("SELECT * FROM courses ORDER BY course_code ASC");
?>
...
<body>
    ...
    <aside class="sidebar">
        <!-- sidebar close btn -->
        <button type="button"
            class="sidebar-close-btn text-gray-500 hover-text-white hover-bg-main-600 text-md w-24 h-24 border border-gray-100 hover-border-main-600 d-xl-none d-flex flex-center rounded-circle"><i
            ...
            <div class="p-20 pt-10">
                <ul class="sidebar-menu">
                   ...
                   <li class="sidebar-menu__item has-dropdown">
                        ...
                        <ul class="sidebar-submenu">
                            <li class="sidebar-submenu__item">
                                <a href="re.php" class="sidebar-submenu__link"> Institution Type </a>
                            </li>
                            <li class="sidebar-submenu__item">
...
<button type="button" class="btn btn-success" data-bs-toggle="modal" data-bs-target="#addModal">Add</button>
        ...
        <form method="post" id="deleteForm">
            <div class="table-responsive mt-12" style="max-height: 500px; overflow-y: auto;">
                <table id="allocationTable" class="table table-bordered table-striped align-middle text-center mt-12 mb-12" style="font-size:15px;">
    ...
    <tr>
        ...
        <th>Actions</th>
                    ...
                    <tbody>
                   <?php while($row = $allocs->fetch_assoc()): ?>
    <tr>
    ...
    data-id="<?= $row['id'] ?>"
    data-regulation="<?= htmlspecialchars($row['regulation']) ?>"
    data-academic_year="<?= htmlspecialchars($row['academic_year']) ?>"
    data-batch="<?= htmlspecialchars($row['batch']) ?>"
    data-institution_code="<?= htmlspecialchars($row['institution']) ?>"
    data-course_code="<?= htmlspecialchars($row['course']) ?>"
>Edit</button>
            <button type="button" class="btn btn-danger btn-sm deleteBtn" data-id="<?= $row['id'] ?>">Delete</button>
        </td>
    </tr>
<?php endwhile; ?>
                    </tbody>
    ...
    <div class="modal-content">
      <form id="addForm" method="post">
        ...
        <div class="modal-body">
          <label for="regulation" class="form-label">Regulation</label><br>
          <select name="regulation" id="add_regulation" class="form-control mb-2" required>
            <option value="">Select Regulation</option>
            <?php while($reg = $regulations->fetch_assoc()): ?>
              <option value="<?= htmlspecialchars($reg['regulation']) ?>"><?= htmlspecialchars($reg['regulation']) ?></option>
            <?php endwhile; ?>
          </select>
          <label for="academic_year" class="form-label">Academic Year</label><br>
          <select name="academic_year" id="add_academic_year" class="form-control mb-2" required>
          ...
          <select name="batch" id="add_batch" class="form-control mb-2" required>
            <option value="">Select Batch</option>
            <?php while($batch = $batches->fetch_assoc()): ?>
              <option value="<?= htmlspecialchars($batch['batch_name']) ?>"><?= htmlspecialchars($batch['batch_name']) ?></option>
            <?php endwhile; ?>
          </select>
          <label for="institution_code" class="form-label">Institution Code</label><br>
          <select name="institution_code" id="add_institution_code" class="form-control mb-2" required>
  <option value="">Select Institution</option>
  <?php while($inst = $institutions->fetch_assoc()): ?>
    <option value="<?= htmlspecialchars($inst['code'] . ', ' . $inst['name']) ?>">
      <?= htmlspecialchars($inst['code']) ?>, <?= htmlspecialchars($inst['name']) ?>
    </option>
  <?php endwhile; ?>
</select>
<label for="course_code" class="form-label">Course Code</label><br>
<select name="course_code" id="add_course_code" class="form-control mb-2" required>
  <option value="">Select Course</option>
  <?php while($course = $courses->fetch_assoc()): ?>
    <option value="<?= htmlspecialchars($course['course_code'] . ', ' . $course['course_title']) ?>">
  ...
  <?php endwhile; ?>
    ...
    <!-- Edit Modal (similar to Add Modal, but with values filled via JS) -->
<div class="modal" id="editModal">
  ...
  <label for="regulation" class="form-label">Regulation</label><br>
  <select name="regulation" id="edit_regulation" class="form-control mb-2" required>
    <option value="">Select Regulation</option>
    <?php
    $regulations2 = $mysqli->query("SELECT * FROM regulation ORDER BY regulation ASC");
    while($reg = $regulations2->fetch_assoc()): ?>
      <option value="<?= trim(htmlspecialchars($reg['regulation'])) ?>">
    <?= htmlspecialchars($reg['regulation']) ?>
</option>
    <?php endwhile; ?>
  </select>
  <label for="academic_year" class="form-label">Academic Year</label><br>
  <select name="academic_year" id="edit_academic_year" class="form-control mb-2" required>
    <option value="">Select Academic Year</option>
    <?php
    $years2 = $mysqli->query("SELECT * FROM academic_year ORDER BY year_name ASC");
    while($year = $years2->fetch_assoc()): ?>
    <option value="<?= trim(htmlspecialchars($year['year_name'])) ?>">
    <?= htmlspecialchars($year['year_name']) ?>
     <?php endwhile; ?>
  </select>
  <label for="batch" class="form-label">Batch</label><br>
  <select name="batch" id="edit_batch" class="form-control mb-2" required>
    <option value="">Select Batch</option>
    <?php
    $batches2 = $mysqli->query("SELECT * FROM batch ORDER BY batch_name ASC");
    while($batch = $batches2->fetch_assoc()): ?>
    <option value="<?= trim(htmlspecialchars($batch['batch_name'])) ?>">
    <?= htmlspecialchars($batch['batch_name']) ?>
    </option>
    <?php endwhile; ?>
  </select>
  <label for="institution_code" class="form-label">Institution Code</label><br>
  <select name="institution_code" id="edit_institution_code" class="form-control mb-2" required>
  <option value="">Select Institution</option>
  <?php
  $institutions2 = $mysqli->query("SELECT * FROM add_institution ORDER BY code ASC");
  while($inst = $institutions2->fetch_assoc()): ?>
    <option value="<?= htmlspecialchars($inst['code'] . ', ' . $inst['name']) ?>">
      <?= htmlspecialchars($inst['code']) ?>, <?= htmlspecialchars($inst['name']) ?>
    </option>
  <?php endwhile; ?>
  ...
  <label for="course_code" class="form-label">Course Code</label><br>
  <select name="course_code" id="edit_course_code" class="form-control mb-2" required>
  <option value="">Select Course</option>
  <?php
  $courses2 = $mysqli->query("SELECT * FROM courses ORDER BY course_code ASC");
  while($course = $courses2->fetch_assoc()): ?>
    <option value="<?= htmlspecialchars($course['course_code'] . ', ' . $course['course_title']) ?>">
      <?= htmlspecialchars($course['course_code']) ?>, <?= htmlspecialchars($course['course_title']) ?>
    </option>
  <?php endwhile; ?>
...
$('.editBtn').on('click', function(){
    ...
    // Helper function to select option by value (trims spaces)
    function selectOptionByValue(selectId, value) {
        var $select = $('#' + selectId);
        var found = false;
        $select.find('option').each(function(){
            if ($(this).val().trim() === String(value).trim()) {
                $select.val($(this).val());
                found = true;
                return false; // break
            }
        ...
        if (!found) {
            $select.val(''); // fallback to empty
        }
    }

    selectOptionByValue('edit_regulation', $(this).data('regulation'));
    selectOptionByValue('edit_academic_year', $(this).data('academic_year'));
    selectOptionByValue('edit_batch', $(this).data('batch'));
    selectOptionByValue('edit_institution_code', $(this).data('institution'));
    selectOptionByValue('edit_course_code', $(this).data('course'));
