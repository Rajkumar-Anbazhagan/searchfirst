...
if (isset($_POST['add'])) {
    ...
    $mysqli->query("INSERT INTO regulation (regulation, created_at) VALUES ('$regulation', NOW())");
    header('Location: regulation.php'); exit;
}
// Handle Edit
if (isset($_POST['edit'])) {
    $id = (int)$_POST['id'];
    $regulation = $mysqli->real_escape_string($_POST['regulation']);
    $mysqli->query("UPDATE regulation SET regulation='$regulation' WHERE id=$id");
    header('Location: regulation.php'); exit;
}
...
if (isset($_POST['delete'])) {
    ...
    if (is_array($ids)) {
        ...
        $ids_str = implode(',', $ids);
        $mysqli->query("DELETE FROM regulation WHERE id IN ($ids_str)");
    }
    header('Location: regulation.php'); exit;
}
$regulations = $mysqli->query("SELECT * FROM regulation ORDER BY id ASC");
?>
...
<body>
    ...
    <div class="dashboard-main-wrapper">
        ...
        <form method="post" class="mb-3 d-flex gap-2">
            <input type="text" name="regulation" class="form-control w-auto" placeholder="New Regulation" required>
            <button type="submit" name="add" class="btn btn-success">Add</button>
        </form>
        <form method="post" id="deleteForm">
            <div class="table-responsive mt-12" style="max-height: 500px; overflow-y: auto;">
            <table id="regulationTable" class="table table-bordered table-striped mt-12 mb-12">
                <thead>
                    <tr>
                ...
                <tbody>
                <?php while($row = $regulations->fetch_assoc()): ?>
                    <tr>
                        ...
                        <td>
                            <button type="button" class="btn btn-sm btn-primary editBtn" data-id="<?= $row['id'] ?>" data-regulation="<?= htmlspecialchars($row['regulation']) ?>">Edit</button>
                            <button type="submit" name="delete" value="1" class="btn btn-sm btn-danger singleDeleteBtn" onclick="return confirm('Delete this regulation?')" formaction="?ids[]=<?=$row['id']?>">Delete</button>
                        </td>
                    </tr>
                <?php endwhile; ?>
                </tbody>
            ...
            <button type="submit" name="delete" value="1" class="btn btn-danger mt-2" onclick="return confirm('Delete selected regulations?')">Delete Selected</button>
        </form>
    <!-- Edit Modal -->
        ...
        <div class="modal-content">
          <form method="post">
            <div class="modal-header">
              <h5 class="modal-title">Edit Regulation</h5>
              <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body">
              <input type="hidden" name="id" id="editId">
              <input type="text" name="regulation" id="editRegulation" class="form-control" required>
            </div>
    ...
    <style>
    #regulationTable th, #regulationTable td {
            text-align: center;
        }
    #regulationTable td {
