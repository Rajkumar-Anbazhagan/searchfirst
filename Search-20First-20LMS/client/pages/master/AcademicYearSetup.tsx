...
import { CalendarDays, GraduationCap, Settings, Trash2, Edit3, Plus, Users, BookOpen, Calendar, Clock, Eye } from 'lucide-react';
...
export default function AcademicYearSetup() {
  ...
  const formHandler = useFormHandler(
    ['name', 'startDate', 'endDate', 'description', 'totalPrograms', 'totalSemesters'],
    {
      name: '',
      startDate: '',
      endDate: '',
  ...
  const onSubmit = async (data: any) => {
    if (selectedYear && isEditModalOpen) {
      ...
      setAcademicYears(prev => prev.map(year => year.id === selectedYear.id ? updatedYear : year));
  ...
  const handleDelete = () => {
    if (selectedYear) {
      setAcademicYears(prev => prev.filter(year => year.id !== selectedYear.id));
      setSelectedYear(null);
        ...
        <CardContent>
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <span className="text-sm text-blue-600">{Math.round(getCurrentProgress())}% Complete</span>
            </div>
      ...
      <div className="grid gap-6">
        ...
        <div className="grid gap-4">
          {academicYears.map((year) => (
            <Card key={year.id} className="hover:shadow-lg transition-all duration-300 group">
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      {year.description && (
                        <p className="text-sm text-muted-foreground mt-2">{year.description}</p>
                      )}
                    </div>
                  </div>
                    ...
                    <div className="grid grid-cols-3 gap-4 text-center">
                      <div className="text-center">
                        <div className="flex items-center gap-1 text-blue-600">
                          ...
                          <span className="font-semibold">{year.totalStudents.toLocaleString()}</span>
                        </div>
                      ...
                      <div className="text-center">
                        <div className="flex items-center gap-1 text-green-600">
                          ...
                          <span className="font-semibold">{year.totalPrograms}</span>
                        </div>
                      ...
                      <div className="text-center">
                        <div className="flex items-center gap-1 text-purple-600">
                          ...
                          <span className="font-semibold">{year.totalSemesters}</span>
                        </div>
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <Badge variant="outline" className={getStatusColor(selectedYear.status)}>
                      {selectedYear.status}
                    </Badge>
                  </div>
                  <div>
                    <Label className="text-sm font-medium text-gray-500">Duration</Label>
                    <p className="text-sm">{selectedYear.startDate} to {selectedYear.endDate}</p>
                  </div>
                </div>
                <div className="grid grid-cols-3 gap-4">
                  <div>
                    <Label className="text-sm font-medium text-gray-500">Total Students</Label>
                    <p className="text-sm font-semibold">{selectedYear.totalStudents.toLocaleString()}</p>
                  </div>
                  <div>
                    <Label className="text-sm font-medium text-gray-500">Programs</Label>
                    <p className="text-sm font-semibold">{selectedYear.totalPrograms}</p>
                  </div>
                  <div>
                    <Label className="text-sm font-medium text-gray-500">Semesters</Label>
                    <p className="text-sm font-semibold">{selectedYear.totalSemesters}</p>
                  </div>
                </div>
                  ...
                  <div>
                    ...
                    <div className="mt-2">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <span className="text-sm text-gray-600">{Math.round(getCurrentProgress())}% Complete</span>
                      </div>
                      <Progress value={getCurrentProgress()} className="h-2" />
                    </div>
                  </div>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              "{selectedYear?.name}" and all associated data.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
